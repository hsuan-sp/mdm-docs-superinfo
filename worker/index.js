  ...
  try {
    const res = await fetch(`${SB_URL}/rest/v1/login_logs`, {
      ...
      body: JSON.stringify({
        ...
        action: actionType,
        meta_data: {
          method: actionType === "manual" ? "OTP 驗證" : "自動快取",
        },
      }),
    });
...
export default {
  async fetch(request, env, ctx) {
    ...
    const now = Date.now();
    const rateData = RATE_LIMIT_STORE.get(ip) || {
      count: 0,
      reset: now + 60000,
    };
    ...
    if (url.pathname === "/auth/otp" && request.method === "POST") {
      ...
      if (
        !email.match(/@.*\.edu\.tw$/) &&
        !email.endsWith("@superinfo.com.tw")
      ) {
        return new Response(JSON.stringify({ error: "僅限教育網域登入" }), {
          status: 403,
        });
      }
      return await fetch(`${SB_URL}/auth/v1/otp`, {
        ...
        body: JSON.stringify({
          email,
          create_user: true,
          data: { redirect_to: `${url.origin}/login` },
        }),
      });
    }
    ...
    // API: 驗證與紀錄
    if (
      (url.pathname === "/auth/verify" || url.pathname === "/auth/session") &&
      request.method === "POST"
    ) {
      ...
      if (access_token && user) {
        ctx.waitUntil(
          logLogin(
            user,
            request,
            url.pathname === "/auth/session" ? "auto" : "manual",
            SB_URL,
            SB_SERVICE
          )
        );
    ...
    if (url.pathname === "/auth/me") {
      ...
      const user = tokenMatch
        ? await verifyUserWithSupabase(tokenMatch[1], SB_URL, SB_ANON)
        : null;
      return new Response(JSON.stringify({ email: user?.email || null }), {
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const isPublic =
      url.pathname.match(/\.(css|js|png|jpg|jpeg|gif|ico|svg|woff2?|json)$/) ||
      url.pathname === "/login.html";
    if (!isAuth && !isPublic && url.pathname !== "/login")
    ...
    newHeaders.set(
      ...
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://klcskeeqkwkakgzrewtn.supabase.co; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:; connect-src 'self' https://klcskeeqkwkakgzrewtn.supabase.co;"
    );
