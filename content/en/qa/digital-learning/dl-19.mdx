...
1. **Bluetooth Permissions**: Ensure Bluetooth is enabled on both teacher and student devices. In **iPadOS 26**, go to **Settings > Privacy & Security > Bluetooth** and confirm the "Classroom" app permission is toggled **ON**.
...
If connection issues only occur in a specific single classroom, immediately collaborate with the network administrator to check if that specific Access Point (AP) is suffering from client overload or a firmware anomaly causing broadcast packet loss.
