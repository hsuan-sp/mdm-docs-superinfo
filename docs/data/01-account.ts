...
當 Apple 更新服務條款時，系統會在登入 Apple 校務管理 (Apple School Manager, ASM) 時，
跳出「條款與約定」的彈出視窗。請注意以下重點：
...
**實務建議**：
建議學校指派至少 2 位管理員角色的帳號（例如資訊組長/設備組長及一位系統管理師），
...
*   **授權回收**：只要帳號被停用或刪除，該帳號先前取得的所有 App 分配授權都會自動歸還給學校。
    `,
      },
      {
        ...
        answer: `
**標準政策：不建議。學校裝置應優先使用「管理式 Apple 帳號 (Managed Apple Account)」，**
**或在不登入帳號的情況下使用。**
...
**實務建議**：
在全面推行前，建議先手動更改 2-3 位測試人員（如資訊組長）的帳號，
...
2.  **重新產生**：檢查「教育身分憑證」狀態。若已過期，請點選 **「重新產生 (Regenerate)」**。
    這會為全校的教育架構簽發新的兩年期憑證。
3.  **強制推送**：
    *   憑證更新後，Jamf Pro 通常會自動排程更新描述檔。
    *   **手動加速**：若裝置未及時反應，
        請在「全域管理」>「Apple 教育支援」介面中，點選下方相關按鈕以重新發送教育設定檔。
4.  **喚醒裝置**：若部分裝置仍顯示無效，
    請對該群組發送 **「空白推播 (Blank Push)」**，
    強制裝置與 MDM 通訊並下載最新的 DDM（宣告式管理）配置。

**實務建議**：
*   **預先排程**：建議在每兩年的暑假期間，手動進來點選一次更新，避免學期中突然失效影響教學。
*   **檢查管理式 Apple 帳號**：若憑證正常但仍失靈，
    請確認師生的「管理式 Apple 帳號」是否因網域異動或聯合驗證失敗而導致身分無法對齊。
    `,
      },
      {
        id: "acc-12",
...
根據 WWDC25 的最新更新，Apple 在 **系統版本 26** (iOS 26 / macOS 26) 中進一步強化了 \`MigrateDevice\` 指令，
允許裝置從舊 MDM 靜默遷移至新 MDM，但這仍存在顯著風險：

1.  **教育環境的限制**：
    *   **描述檔殘留**：不抹除遷移極易導致舊 MDM 的「限制描述檔」或「教育配置」與新環境衝突，
...
**實務建議**：
為了確保新學期裝置穩定，請優先使用 **「回到服務 (Return to Service)」** 功能。
這是 iOS 17 加入並在版本 26 最佳化的功能：
管理員只需發送一個指令，裝置會自動抹除並利用快取的 Wi-Fi 資訊自動重新註冊。
這能達成「乾淨、穩定且全自動」的遷移，遠優於不抹除資料的轉移方式。
    `,
      },
      {
        id: "acc-13",
...
**實務建議**：
若您的帳號不是聯合驗證帳號，學生忘記密碼時 **「一定要聯絡資訊組」**。
建議資訊組在開學時統一利用批次選取功能，產生成包含暫時密碼的 PDF 或 CSV 發放給師生，以利後續管理。
    `,
      },
      {
        id: "acc-14",
...
以防止因單一設備遺失導致全校管理後台鎖死的災難。
    `,
      },
      {
        id: "acc-21",
        question: "「moemdm」跟「Jamf Pro」是一樣的東西嗎？為什麼學校需要兩個帳號？",
        important: true,
...
*   **性質**：純粹的資料彙整與統計平台。
    您在 moemdm 網頁上修改設定，並不會直接改變 iPad 的行為。
*   **關鍵聯繫**：moemdm 會透過 API 與 Jamf Pro 接軌。
    只要您在 Jamf Pro 設定正確，數據就會自動同步到 moemdm 供教育部查核。

**為什麼要區分？**
*   **資安考量**：教育部不需要、也不應該直接控制每一所學校的設備（這會造成單點崩潰風險）。
*   **權責分明**：學校保有教學自主權（由校方 Jamf 管理），
    教育部則透過 moemdm 執行資產盤點與績效考核。

**實務建議**：
若要檢查設備是否故障或更換 App，請登入 **Jamf Pro**。
