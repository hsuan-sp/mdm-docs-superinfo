    ...
    items: [
      {
        id: "acc-1",
        question:
          "登入 Apple 校務管理 (ASM) 時，系統提示需同意新的條款與約定，這很重要嗎？",
        important: true,
        tags: ["ASM", "條款更新"],
        answer: `
**非常重要，請優先處理。**
...
*   更新 Token 後，建議執行一次 **「同步大量採購內容 (Sync Volume Purchasing Content)」** 以確保授權數量正確。
    `,
      },
      {
        id: "acc-5",
        question:
          "我們學校有使用 Google Workspace / Microsoft 365，可以用這些帳號登入 Apple 服務嗎？",
...
若上述皆正常但仍無法更新，可能是裝置端的 **宣告式裝置管理 (DDM)** 狀態卡住。您可以嘗試重啟裝置，或透過 Jamf 發送「空白推播 (Blank Push)」指令來強迫喚醒裝置的連線程序。
    `,
      },
      {
        id: "acc-10",
        question:
          "學校的 Email 網域更換了（例如從 .edu.tw 改為 .xh.edu.tw），該怎麼更改管理式 Apple 帳號？",
        tags: ["網域變更", "帳號管理", "重大異動"],
        answer: `
...
*   **檢查管理式 Apple 帳號**：若憑證正常但仍失靈，請確認師生的「管理式 Apple 帳號」是否因網域異動或聯合驗證失敗而導致身分無法對齊。
    `,
      },
      {
        id: "acc-12",
        question:
          "【MDM 遷移】更換新 MDM 廠商時，可以「不抹除資料」直接轉移嗎？",
        important: true,
...
若您的帳號不是聯合驗證帳號，學生忘記密碼時 **「一定要聯絡資訊組」**。建議資訊組在開學時統一利用批次選取功能，產生成包含暫時密碼的 PDF 或 CSV 發放給師生，以利後續管理。
    `,
      },
      {
        id: "acc-14",
        question:
          "ASM 中新採購的裝置無法同步到 Jamf Pro？（ADE 伺服器代號續約）",
        important: true,
...
登入 ASM > **「設定」>「使用者登入與目錄同步」**，在已驗證網域旁可查看衝突狀態與剩餘天數。
    `,
      },
      {
        id: "acc-17",
        question:
          "如何結合 Platform SSO (PSSO) 實現 Mac 開機即解鎖 FileVault？",
...
若新舊廠商皆支援 Apple 於 iOS 17.5 引入的遷移 API，在特定條件下可以達成 **「靜默遷移」**。這意味著裝置在更換 MDM 時，內部的 App 無需刪除，授權會直接由 Apple 伺服器後台「重新分派」給新的 MDM 伺服器，使用者端完全無感。
    `,
      },
      {
        id: "acc-19",
        question:
          "如何控管「管理式 Apple 帳號」是否能使用 Apple Intelligence 功能？",
...
強烈建議所有具備「管理員 (Administrator)」職務的帳號，必須綁定至少兩個以上的救援管道，或使用實體硬體安全金鑰 (Security Keys)，以防止因單一設備遺失導致全校管理後台鎖死的災難。
    `,
      },
      {
        id: "acc-21",
        question:
          "「moemdm」跟「Jamf Pro」是一樣的東西嗎？為什麼學校需要兩個帳號？",
        important: true,
...
*   **關鍵聯繫**：moemdm 會透過 API 與 Jamf Pro 接軌。只要您在 Jamf Pro 設定正確，數據就會自動同步到 moemdm 供教育部查核。
...
**專家建議**：
若要檢查設備是否故障或更換 App，請登入 **Jamf Pro**。若要查看學校的 KPI 是否達標或填寫期末報表，請登入 **moemdm**。
    `,
      },
    ],
  },
