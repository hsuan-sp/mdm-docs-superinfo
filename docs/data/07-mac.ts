...
*   現代 Mac 管理是「學期末/專案結束時」執行一次 EACS 重置。
    ...
    `,
      },
      {
        ...
        answer: `
**Apple Silicon (M1/M2/M3/M4) 的安全性架構與 Intel Mac 完全不同。**
**管理核心在於「Bootstrap Token」與「擁有權 (Ownership)」。**
...
1.  **Bootstrap Token (引導代令)**：
    *   **問題**：在 Apple Silicon 上，部分高權限操作（如安裝軟體更新、啟用核心延伸功能）
        需要「安全代令」。
    *   **解法**：確認 Jamf Pro 的 **PreStage Enrollment** 已勾選「允許 MDM 上傳 Bootstrap Token」。
    *   **驗證**：在 Jamf Pro 電腦紀錄中，確認 \`Bootstrap Token Allowed\` 為 \`Yes\`。
        若為 No，許多管理指令會失敗。
...
3.  **核心延伸功能 (Kernel Extensions / KEXTs)**：
    ...
    *   **現代建議**：請改用 **系統延伸功能 (System Extensions)**，這是 Apple 推薦的新架構，
        可直接透過 MDM 設定檔授權，無需降低系統安全性。
    `,
      },
      {
        id: "mac-8",
...
*   **機構復原密鑰 (IRK)**：現代管理建議優先使用「個人復原密鑰 (PRK)」機制，安全性較高。
    `,
      },
      {
        ...
        answer: `
**腳本 (Scripts) 讓管理員能以 Root 權限在 Mac 上執行 Shell 指令，**
**實現標準 MDM 描述檔 (Payload) 無法達成的進階客製化功能。**

Jamf Pro 的強大之處在於其專屬的二進位檔案 (Binary)，配合 **「政策 (Policies)」** 使用，
可達成以下進階場景：
...
再上傳至 Jamf Pro。
    `,
      },
      {
        ...
        answer: `
**現代化 Mac 列印部署應優先採用「AirPrint (免驅動)」協定。**
**若需進階設定，則透過「政策 (Policies)」或 Shell 腳本執行 \`lpadmin\` 指令。**

在過去，Mac 管理員需花費大量時間打包驅動程式 (PPD)。但在 macOS 12 Monterey 之後，
Apple 大力推動 IPP/AirPrint 通用協定，大幅簡化了流程。
...
1.  **新增印表機**：
    *   前往 **「設定」** > **「電腦管理」** > **「印表機」**。
    *   點選 **「+ 新增」**，輸入 IP 位址與顯示名稱。
    *   **關鍵設定**：在「驅動程式」欄位，若印表機支援 AirPrint，可選擇 **「Generic PCL Laser Printer」**
...
*   **解法**：將每台印表機做成一個 **「自助服務」** 項目，並附上位置說明
    ...
    `,
      },
      {
        ...
        answer: `
**最佳實踐是使用「Jamf App Installers」自動部署安裝檔，**
**並依據使用場景區分「序列化 (Serializer)」或「使用者登入」兩種啟用方式。**

請務必使用 Microsoft 官方 CDN 版本（或透過 Jamf 派送），**嚴禁使用 Mac App Store 版本**，
否則將無法支援大量授權功能。
...
**部署 SOP (Jamf Pro)：**
1.  **設定 App Installers**：在「電腦管理」>「Mac App」中啟用 Microsoft Office 365
    (包含 Teams/Outlook 等)。
2.  **建立 Serializer 政策 (僅限教室)**：將 VL Serializer 設為「Once per computer」
    並派送給教室群組。
3.  **設定 MAU (Microsoft AutoUpdate)**：建立組態設定檔，強制設定 MAU 的更新期限 (Deadline)，
    確保漏洞被修補。
...
清除乾淨後再重新部署。
    `,
      },
      {
        ...
        answer: `
**macOS 15 Sequoia 引入的「iPhone 鏡像輸出」雖然方便，**
**但在校園公用電腦或高機密行政機上存在資料外洩 (DLP) 風險，建議透過 MDM 限制。**

**資安風險：**
當使用者將個人 iPhone 與學校 Mac 配對後，iPhone 的通知、照片與 App 內容可直接投射在 Mac 螢幕上，
