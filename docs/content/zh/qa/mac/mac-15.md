...
3. **設定 App Installers**：在「電腦管理」>「Mac App」中啟用 Microsoft Office 365

 (包含 Teams/Outlook 等)。

4. **建立 Serializer 政策 (僅限教室)**：將 VL Serializer 設為「Once per computer」

 並派送給教室群組。

5. **設定 MAU (Microsoft AutoUpdate)**：建立設定描述檔，強制設定 MAU 的更新期限 (Deadline)，
