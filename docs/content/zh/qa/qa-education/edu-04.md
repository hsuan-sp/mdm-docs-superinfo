...
***情境一：共用 iPad 模式（最適合頻繁輪替）**若裝置已設定為「共用 iPad」：
...
***情境二：訪客模式（共用 iPad 的臨時使用）**若來訪學生沒有帳號：
...
***情境三：一般 iPad（非共用模式）的快速清理**若裝置不是共用 iPad 模式（個人載具臨時借用）：
...
***情境四：徹底重置 (Return to Service - 2026 進化版）**若需要完全還原至乾淨狀態，且希望下一班能「立即開機即用」：
