...
***徹底清除**：確保裝置回到乾淨狀態***保留 Wi-Fi**：重置後自動連線，無需手動設定***自動註冊**：自動完成 ADE 流程***保留 App（iOS 26+）**：可選擇保留 Managed App 的程式本體，節省重新下載時間***適用時機**：
...
***一對一配發的教職員裝置**：避免行政同仁重新設定個人化設定和資料***BYOD 裝置轉換管理**：員工自有裝置更換管理方式時，保留個人資料***測試與驗證**：小規模測試新 MDM 系統時，快速驗證功能***不建議使用的情況**：
...
5. 向使用者說明遷移過程和時程***假設選擇 Return to Service**：
