...
6. 登入 Jamf Pro 管理介面。
7. 前往 **「設定 (Settings)」>「全域管理 (Global Management)」>「大量採購 (Volume Purchasing)」**。
8. 找到對應的位置 (Location) 項目，點選進入編輯畫面。
9. 點選 **「上傳 (Upload)」**，選擇剛才下載的 .vpptoken 檔案。
10. 儲存後，狀態應顯示為 **「有效 (Active)」** 並且正確更新過期時間。
