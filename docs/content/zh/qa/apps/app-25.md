...
## Answer**macOS 26 (Tahoe) 將非 App Store 軟體 (.pkg) 納入宣告式裝置管理 (DDM) 範疇，允許透過 Declaration 部署軟體包，提供比傳統 Jamf Policy 更現代化、更透明的安裝體驗。**## DDM Package vs. Jamf Policy
...
## 適用條件與限制**macOS 版本需求：*** macOS 26 (Tahoe) 或更新版本**.pkg 檔案要求：*** 必須由**Apple Developer ID Installer certificate**正確簽署
* 必須為標準的 Distribution Package 格式
* 安裝路徑需符合 macOS 安全性要求（通常為 `/Applications` 資料夾）

## 何時該用哪一個？**優先使用 DDM Package：*** 標準軟體安裝（如 Microsoft Office、Google Chrome、Adobe Acrobat）
* 需要即時確認安裝成功率的關鍵軟體
* macOS 26 以上的新電腦
