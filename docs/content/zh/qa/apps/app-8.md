...
**2. 自訂圖示與視覺*****建議規格**：上傳**180x180 像素的 PNG 檔案**以獲得最佳的清晰度。

***透明度支援**：現代系統支援帶有透明背景的 PNG 圖示，這能讓桌面佈置更具設計感。

**3. 防止移除 (Non-removable)***對於校用公用機，您可以將 Web Clip 設定為「不可移除」。

***優點**：確保必備的教學入口不會被學生誤刪，免去重複派送的麻煩。

### 限制與風險提示***瀏覽器依賴**：Web Clip 預設依賴內建的 Safari 引擎。若您在限制描述檔中徹底停用了 Safari 瀏覽器，Web Clip 將會無法開啟。
