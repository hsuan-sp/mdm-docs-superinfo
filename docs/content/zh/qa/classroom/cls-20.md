...
***MDM 無法偵測或禁止學生「設定」鬧鐘，但可以透過「隱藏 App」來根絕此問題。*****為什麼管不到？**Apple 的 MDM 協定並未開放「讀取或修改鬧鐘設定」的權限。
...
***注意：若學生在隱藏前已設定了鬧鐘，時間到時仍可能會響，此時需重置設定。**1.**還原設定 (Reset All Settings)**：

* 若鬧鐘已經設好且隱藏 App 後仍在響，

 請執行裝置端的**「重置所有設定」**（不會刪除資料，但會移除鬧鐘）。

2.**使用 Jamf Teacher 的「App 限制」**：
