...
* *This involves a major underlying system change.**
* *Since the format of a Managed Apple Account changes with the domain, it directly affects user login identities.**
...
5. Export all users as a CSV from the "Users" list.
6. Find & Replace the domain part of the "Managed Apple Account" column in the CSV with the new domain.
7. Re-upload the updated CSV via the import function.

* **For Federated Authentication (Google / Microsoft)**:

8. **Critical Order**: You must first complete the email domain migration on the Google or Microsoft side.
9. Return to ASM to trigger a re-sync.
10. ASM will detect the email changes and automatically update the corresponding Managed Apple Accounts.
