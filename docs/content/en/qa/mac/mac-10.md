...
***Paste your Shell Script in the editor.*****Crucial Syntax**: Jamf Pro only reads values enclosed in `<result>`and`</result>`tags. Any other`echo` output will only appear in debug logs and won't be written to the database.
