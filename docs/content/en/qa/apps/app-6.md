...
***The license is then returned to the school's VPP (Volume Purchase Program) pool for the next student to use.*****Data Risk**: If the "Remove app when management profile is removed" setting is enabled, all data stored within the app (e.g., unsaved projects, local notes) will be**permanently erased**.
