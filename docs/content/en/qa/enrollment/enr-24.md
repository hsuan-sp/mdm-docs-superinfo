...
***The device manages its own notification frequency locally.*****At the deadline, the action is forced without needing the MDM server to push a real-time command.**## Scope
...
***Choose the time zone (Local device time is recommended).**5. Deploy to the target group.
