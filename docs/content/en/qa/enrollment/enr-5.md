...
tags:
  ...
  - Technical Setup
title: >-
  How do we set up 'Service Discovery' for Account-Driven Enrollment on the
  campus network?
---
...
## Technical Requirements:***Web Server Access**: Your school’s main website server must be configured to host this specific `.well-known` directory.***MIME Type**: The server must return the file with the `application/JSON` header.***SSL Certificate**: The server MUST have a valid, trusted HTTPS certificate.
