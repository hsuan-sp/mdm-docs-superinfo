...
---
## Term Definition***QR Code Enrollment**is a streamlined method for enrolling mobile devices into an MDM environments.

The process:***Scan**: The user points the device camera at a unique QR code provided by IT.***Decode**: The device reads the embedded URL and Enrollment token.***Connect**: Safari automatically opens to the specific Enrollment portal.***Install**: The user is prompted to download and install the MDM Profile immediately.
