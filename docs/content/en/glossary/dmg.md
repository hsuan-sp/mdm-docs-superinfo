...
term: DMG (Disk Image)
---
## Term Definition***DMG**is a proprietary Apple Disk Image file format used primarily for software distribution on macOS.

Key characteristics:***Mounting**: When opened, it behaves like a virtual external drive or CD-ROM appearing on the Desktop.***Compression**: Often compressed to save space during download.***Usage**: Typically used to deliver `.app`bundles. Users install software by dragging the application icon from the DMG into the`/Applications` folder.
