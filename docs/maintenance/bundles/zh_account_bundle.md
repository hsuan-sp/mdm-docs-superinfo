...
當 Apple 更新服務條款時，系統會在登入 Apple 校務管理 (Apple School Manager, ASM) 時，跳出「條款與約定」的彈出視窗。請注意以下重點：
...
- 若未同意條款，您將無法存取 Apple 校務管理中的新功能，例如 **Apple Intelligence 控制項** 可能需要透過 ASM 條款同意才能在 MDM 中使用，或 **查看裝置 AppleCare 保養範圍資訊** 。
...
1. 等待數秒後，或嘗試主動點擊上方出現的橫幅，系統會彈出條款視窗。
1. 請仔細閱讀後勾選「我已閱讀並同意...」。
1. 勾選所有項目然後點選「同意」按鈕即可完成。

## 實務建議 ：

建議學校指派至少 2 位管理員角色的帳號（例如 資訊組長/設備組長及一位系統管理師），以確保當主要負責人請假或離職時，仍有人可以處理這類緊急事項，確保管理不中斷。
...
1. **選取多位學生** ：使用鍵盤組合鍵進行選取（按住 **Command** 或 **Ctrl** 鍵不放，再點選清單中的使用者；或按住 **Shift** 進行連續選取）。
1. 選取完成後，右側面板會顯示「已選取 X 位使用者」。
1. 點選右側面板中的 **「建立登入資訊」** 按鈕。
1. 在彈出的視窗中，點選下載為 **「PDF」** 或 **「CSV」** 。這份檔案會列出該批學生的姓名及其對應的 **暫時密碼** ，方便列印發放。
...
1. 點選左側選單的 **「使用者」** ，搜尋並選取該學生。
1. 在右側面板中點選 **「變更帳號狀態」** (或是點選狀態旁邊的「編輯」/「變更」)。
1. 將狀態改為 **「停用」** 。
...
1. 使用鍵盤組合鍵（按住 **Command** 或 **Ctrl** ，或使用 **Shift** 連選）選取多位學生。
1. 點選右側面板出現的 **「變更帳號狀態」** ，即可一次將整班改為「停用」。
...
>
> 1. **僅限 App Store 登入** ：請至「設定」>「App Store」登入個人帳號。 **而不要** 在「設定」最頂端的 iCloud 區塊登入。
...
管理式 Apple 帳號的格式隨網域更換而變動時，將直接影響使用者的登入身分。
...
- **若已啟用聯合驗證（Google / Microsoft）** ：
  ...
  2. 回到 ASM 觸發重新同步。
  3. ASM 會辨識到 Email 變更並自動更新對應的「管理式 Apple 帳號」。
...
- **共用 iPad (Shared iPad)** ：若有部署「共用 iPad」，登入畫面會隨網域變更自動更新，學生只需輸入新帳號即可。

## 實務建議 ：

在全面推行前，建議先手動更改 2-3 位測試人員（如 資訊組長）的帳號，確認在新舊網域切換後，iCloud 資料與 MDM 指令是否都能正常運作。
...
根據 WWDC 25 的最新更新，Apple 在 **系統版本 26** (iOS 26 / macOS 26) 中進一步強化了 `MigrateDevice` 指令，允許裝置從舊 MDM 靜默遷移至新 MDM，但這仍存在顯著風險：

## 1. 教育環境的限制 ：

- **描述檔殘留** ：不抹除遷移極易導致舊 MDM 的「限制描述檔」或「教育設定」與新環境衝突，導致 Apple 課堂 (Classroom) 失靈。
...
為了確保新學期裝置穩定，請優先使用 **「回到服務 (Return to Service, RTS)」** 功能。這是 iOS 17 加入並在 **版本 26** 最佳化的功能：

管理員只需發送一個指令，裝置會自動抹除並利用快取的 Wi-Fi 資訊自動重新註冊，且 **OS 26 支援在 RTS 過程中保留已安裝的受管應用程式** 。這能達成「乾淨、穩定且全自動」的遷移，是目前最高效的轉移方式。
...
若您的帳號不是聯合驗證帳號，學生忘記密碼時 **「一定要聯絡 資訊組」** 。
建議 資訊組在開學時統一利用批次選取功能，產生成包含暫時密碼的 PDF 或 CSV 發放給師生，以利後續管理。
...
3.  **60 天寬限期** ：使用者有 60 天的時間來決定：
    - **更改個人帳號 Email** ：將帳號改為 Gmail 或其他個人信箱，原本的資料（照片、App 購買紀錄）會完整保留在該帳號中，但帳號名稱會變更。
    - **配合釋放** ：若該帳號純屬辦公教學使用，可配合變更個人帳號 Email，以便由組織重新將該 Email 建立為「管理式 Apple 帳號 (Managed Apple Account)」。
4.  **強制釋出 (Domain Capture)** ：若 60 天後使用者未做動作，Apple 會強制要求使用者在登入時更改 Email（系統會自動產生一個暫時的 @gtempaccount.com 或類似名稱），屆時該學校 Email 才會被釋放，供學校重新建立為管理式帳號。在 2025 年後的版本中，管理員可更進階地透過「網域擷取」功能自動化此流程。
