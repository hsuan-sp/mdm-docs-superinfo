...
const MobileNav: React.FC<MobileNavProps> = ({ isOpen, onClose }) => {
  ...
  const resourceGroups = translations[language].resources.groups as ResourceGroup[];
    ...
    <div className="fixed inset-0 z-10000 lg:hidden text-balance shadow-2xl">
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 bg-white/40 dark:bg-zinc-950/40 h-20 shrink-0 border-b border-zinc-100 dark:border-zinc-900">
          ...
          <button onClick={onClose} className="p-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-all active:scale-90 border border-transparent hover:border-zinc-200 dark:hover:border-zinc-700 min-h-11 min-w-11 flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue" aria-label="Close menu">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-10 no-scrollbar">
          ...
          <div className="p-1.5 bg-zinc-50 dark:bg-zinc-900/50 rounded-5xl border border-zinc-100 dark:border-zinc-800/50 shadow-inner">
            {user ? (
              <div className="flex flex-col p-2 space-y-2">
                ...
                <button onClick={() => signOut()} className="w-full flex items-center justify-center gap-2.5 px-6 py-3.5 text-red-500 dark:text-red-400 font-bold text-[13px] hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all active:scale-95 min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue">
                  <LogOut className="w-4 h-4 opacity-70" />
                  {t('userCenter.logout')}
                </button>
              </div>
            ) : (
              <button onClick={() => signIn()} className="w-full flex items-center justify-center gap-3 px-6 py-4.5 bg-apple-blue text-white rounded-4xl font-bold text-[15px] hover:bg-apple-blue-hover shadow-xl shadow-apple-blue/20 active:scale-95 transition-all min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-apple-blue">
                {t('userCenter.login')}
          ...
          <div className="pt-6 border-t border-zinc-100 dark:border-zinc-900 pb-12 space-y-3">
              <button onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')} className="w-full flex items-center justify-between p-4.5 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl transition-all hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-[0.97] min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue">
                <div className="flex items-center gap-3.5">
              ...
              <button onClick={() => setLanguage(language === 'zh-TW' ? 'en' : 'zh-TW')} className="w-full flex items-center justify-between p-4.5 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl transition-all hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-[0.97] min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue">
                <div className="flex items-center gap-3.5">
