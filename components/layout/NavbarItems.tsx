  ...
  return (
    <div className="flex items-center gap-3.5 select-none group branding-logo-container relative cursor-pointer">
      ...
      <div className="flex flex-col justify-center">
        <span
          className="font-bold text-zinc-950 dark:text-zinc-50 whitespace-nowrap leading-tight tracking-tight transition-colors group-hover:text-apple-blue w-[100px]"
          style={{ fontSize: "clamp(15px, 2.5vw, 19px)" }}
        >
          {isZh ? "極電資訊" : "Superinfo"}
        </span>
        <span
          className="font-bold text-zinc-400 dark:text-zinc-500 whitespace-nowrap leading-none mt-1 tracking-widest"
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              key={link.path}
              href={link.path}
              className={`w-28 flex items-center justify-center px-2 py-2 rounded-full text-[13px] font-bold transition-all duration-300 relative group overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue ${
                isActive
      ...
      <div className="hidden lg:flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className={`w-[130px] flex items-center justify-between px-4 py-2 hover:bg-zinc-100/80 dark:hover:bg-zinc-800/80 rounded-full transition-all text-[13px] font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue ${resourcesOpen ? "text-apple-blue bg-apple-blue/5 dark:bg-apple-blue/10" : "text-zinc-600 dark:text-zinc-400"}`}
            aria-label="Toggle resources menu"
            aria-expanded={resourcesOpen}
          >
            <span className="truncate">{t("resources.label")}</span>
            <ChevronDown
              ...
              <div className="absolute right-0 mt-3 w-80 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-zinc-200/50 dark:border-zinc-800/50 z-50 overflow-hidden animate-reveal p-2">
                {resourceGroups.map((group: ResourceGroup, idx: number) => (
                  <div key={idx} className="mb-1 last:mb-0">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-400/80 px-4 py-2.5">
                      {group.title}
                    </h3>
                    <div className="space-y-0.5">
                      {group.items.map(
                        (item: ResourceItem, itemIdx: number) => (
                          <a
                            key={itemIdx}
                            href={item.link}
                            target="_blank"
        ...
        <button
          ...
          className="p-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-all active:scale-90 relative group min-h-11 min-w-11 flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-apple-blue overflow-hidden"
          aria-label={
            resolvedTheme === "dark"
